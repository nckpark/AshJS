    ...
    for( var i = 0; i < _gameNodes.length(); i++ ) {
      ...
      if( _spaceships.empty() ) {
        if( gameNode.state.lives > 0 ) {
          var newSpaceshipPosition = new createjs.Point( config.width * 0.5, config.height * 0.5 );  // ghetto to use global config like that. don't do that in the future
      ...
      if( _asteroids.empty() && !_spaceships.empty() ) {
        ...
        for( var k = 0; k < asteroidCount; ++k ) {
          ...
          var asteroidPos;
          do {
            asteroidPos = new createjs.Point( Math.random() * config.width, Math.random() * config.height ); 
...
GameManager.prototype = new ash.System();
